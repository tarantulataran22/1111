...
import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("Coded By Script & Kamma")
import aiohttp
import colorama
import discord
import numpy
...
stream_url = "https://www.twitch.tv/suwox3"
tts_language = "en"
...
{Fore.RED}▀███████████ ███        ▀▀███▀▀▀▀▀   ███▌ ▀█████████▀      ███   
 ...
 {Fore.RED}                         ███    ███           
                  ...
                  {Fore.RED}[{Fore.RESET}Prefix{Fore.RED}] {Fore.WHITE}{Nax.command_prefix}     {Fore.RED}[{Fore.RESET}Note{Fore.RED}]  {Fore.WHITE}Script & Kamma
...
def Init():
    ...
    try:
        ...
        os.system(f'title (ZoT Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Nax = discord.Client()
Nax = commands.Bot(description='ZoT Selfbot', command_prefix=prefix, self_bot=True)
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Nax.whitelisted_users.keys() and i.user.id in Nax.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="EnVy Anti-Nuke")
        ...
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Nax.whitelisted_users.keys() and i.user.id in Nax.whitelisted_users[
                    ...
                    await guild.ban(member, reason="EnVy Anti-Nuke")
                    await guild.ban(i.user, reason="EnVy Anti-Nuke")
        except Exception as e:
            print(e)
...
async def nax(ctx, *, nax=None):
    await ctx.message.delete()
    await ctx.send("`SelfBot by !`Script Akar#0082`")
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if Nax.yui_kiss_user is None or Nax.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact !`Script Akar#0082')
            return
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="get fucked LOL",
            reason="EnVy me#8054 LOL",
            icon=None,
            banner=None
        )
    except:
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="EnVy")
    for _i in range(250):
        await ctx.guild.create_role(name="EnVy", color=RandomColor())
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="EnVy me#8054")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="EnVy me#8054")
        except:
            pass
...
async def massrole(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="EnVy me#8054", color=RandomColor())
        except:
            return
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="EnVy me#8054")
        except:
            return
